...
!! 
subroutine lusgs_hyb_col(gamma,rey,q,s,tscale,timeMetric,dx,dy,dz,nq,nvar,jmax,kmax,lmax)
!
!
implicit none
!
...
real*8, intent(in) :: q(jmax,kmax,lmax,nq)
real*8, intent(inout) :: s(jmax,kmax,lmax,nq)
real*8, intent(in) :: timeMetric(jmax,kmax,lmax,3)
...
do k=ks,ke
   do l=ls-1,le+1
      do j=js-1,je+1
         ...
         vv=q(j,k,l,3)*rhoi
         ww=q(j,k,l,4)*rhoi
         ug=timeMetric(j,k,l,1)
         vg=timeMetric(j,k,l,2)
         wg=timeMetric(j,k,l,3)
         uvw=0.5*(uu*uu+vv*vv + ww*ww)
         cjkl=sqrt(ggm1*(q(j,k,l,5)*rhoi-uvw))
         !
         qq1=uu*dxi
         qqx=abs((uu-ug)*dxi)
         rr1=dxi
         ...
         svt=tscale
         if (nq.gt.5) then
            vscale=(1d0+q(j,k,l,6))/(rey*q(j,k,l,1))
         else
            vscale=1d0/(rey*q(j,k,l,1))
         endif
         !
         ...
         cx(j,l)=cjkl*rr1
         cz(j,l)=cjkl*rr3
         ge(j,l)=gamma*q(j,k,l,5)*rhoi-(gamma-1)*uvw
	 if (msweep.eq.1) then
          do n=1,nvar
            s(j,k,l,n)=s(j,k,l,n)*svt
          enddo
         endif
      enddo
   enddo
   ...
   do l=ls-1,le+1
      do j=js-1,je+1
         ...
         uvw=0.5*(uu*uu+vv*vv+ww*ww)
         !
         ug=timeMetric(j,kr,l,1)
         vg=timeMetric(j,kr,l,2)
         wg=timeMetric(j,kr,l,3)
         !
         rr2=dyi*dyi
         qq=vv*dyi
   ...
   do m=js+ls,je+le
      do j=ms(m),me(m)
         ...
         qq=qx(j1,l)
         cc=cx(j1,l)
         ug=timeMetric(j1,k,l,1)
         vg=timeMetric(j1,k,l,2)
         wg=timeMetric(j1,k,l,3)
         qqx=-dxi*ug+qq
         sp1=abs(qqx)+cc
         sm2=eps2*sp1
      ...
      do j=ms(m),me(m)
         ...
         uvw=uv(j,l1)
         !
         ug=timeMetric(j,k,l1,1)
         vg=timeMetric(j,k,l1,2)
         wg=timeMetric(j,k,l1,3)
         qq=qz(j,l1)
         cc=cz(j,l1)
         !
         qqz=-dzi*wg+qq
   ...
   !
      do j=ms(m),me(m)
       ...
       !
       do n=1,nvar
          s(j,k,l,n)=(s(j,k,l,n) + svt*0.5*(a(n,j)+b(n,j,l)+c(n,j)))*d(j,k,l)
       enddo
       !
     enddo
   enddo
